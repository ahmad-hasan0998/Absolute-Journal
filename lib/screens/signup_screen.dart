...
class _SignupScreenState extends State<SignupScreen> {
  ...
  void _showModernSnackbar(String message, bool isError) {
    ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          ...
          margin: EdgeInsets.only(bottom: MediaQuery.of(context).size.height * 0.8, left: 24, right: 24),
          elevation: 10,
        )
    );
  }
  ...
  void _handleSignUp() async {
    ...
    try {
      var user = await AuthService().signUpWithEmail(_emailController.text.trim(), _passwordController.text.trim(), _usernameController.text.trim().toLowerCase());
      if (mounted) setState(() { _isLoading = false; });
      if (user != null && mounted) {
        AuthService().signOut();
        _showModernSnackbar('Account created successfully! Please log in.', false);
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              const Text('Create\nAccount', style: TextStyle(fontSize: 40, fontWeight: FontWeight.w900, color: Color(0xFF0A2463), height: 1.1)),
              const SizedBox(height: 40),
              _buildShadowedInput(hint: 'Username (lowercase only)', icon: Icons.person_outline, controller: _usernameController),
