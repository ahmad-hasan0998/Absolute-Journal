...
class DashboardScreen extends StatelessWidget {
  ...
  Future<bool?> _showRatingDialog(BuildContext context, String title) {
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    ElevatedButton.icon(
                      ...
                      onPressed: () async {
                        if (item.progress + selectedAmount >= item.total) {
                          bool? liked = await _showRatingDialog(
                            context,
                            item.title,
                          );
                          if (liked != null) {
                            item.isLiked = liked;
                            context.read<JournalProvider>().incrementProgress(
                              item,
                              selectedAmount,
                            );
                    ...
                    TextButton(
                      onPressed: () async {
                        ...
                        if (liked != null) {
                          item.isLiked = liked;
                          context.read<JournalProvider>().completeEntireShow(
                            item,
                          );
                          Navigator.pop(context);
                        }
  ...
  void _showCardFocusDialog(BuildContext context, MediaItem item) {
    showDialog(
      ...
      builder: (context) {
        return StatefulBuilder(
          builder: (context, setDialogState) {
            double completionRatio = item.total > 0
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        GestureDetector(
                          onTap: () {
                            item.isLiked = (item.isLiked == true) ? null : true;
                    ...
                    ListTile(
                      ...
                      onTap: () {
                        context.read<JournalProvider>().removeMedia(item);
                        Navigator.pop(context);
                      },
                    ),
                  ],
  ...
  Widget _buildTrailingWidget(BuildContext context, MediaItem item) {
    if (item.type == 'Movie') {
      return GestureDetector(
        onTap: () async {
          bool? liked = await _showRatingDialog(context, item.title);
          if (liked != null) {
            item.isLiked = liked;
            context.read<JournalProvider>().completeEntireShow(item);
          }
        },
        child: Container(
    ...
    return GestureDetector(
      onTap: () async {
        if (item.progress + 1 >= item.total) {
          bool? liked = await _showRatingDialog(context, item.title);
          if (liked != null) {
            item.isLiked = liked;
            context.read<JournalProvider>().incrementProgress(item, 1);
          }
        } else {
          context.read<JournalProvider>().incrementProgress(item, 1);
        }
  ...
  Widget build(BuildContext context) {
    ...
    final dashboardItems = activeItems
        .where((item) => item.status != 'Completed' && !item.isWatchLater)
        .toList();
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              if (item.type != 'Movie') ...[
                                ...
                                const SizedBox(height: 8),
                                Text(
                                  '${item.progress} / ${item.total} Episodes',
                                  style: const TextStyle(
