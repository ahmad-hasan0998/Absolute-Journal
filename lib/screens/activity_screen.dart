            ...
            return SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _buildRuleRow(
                          ...
                          '-50 XP',
                          'Per day missed (after 24h grace period)',
                        ),
                      ],
                    ),
                  ),
                  ...
                  const SizedBox(height: 16),
                  ...ranks.map((r) {
                    final String rName = r['name'] as String;
                    final int rXp = r['xp'] as int;
                    ...
                    return Container(
                      ...
                      decoration: BoxDecoration(
                        color: isCurrent
                            ? rColor.withValues(alpha: 0.1)
                            : Colors.white,
                        borderRadius: BorderRadius.circular(20),
                        border: isCurrent
                            ? Border.all(color: rColor, width: 2)
            ...
            body: TabBarView(
              children: [
                ...
                FutureBuilder<List<Map<String, dynamic>>>(
                  future: locator<DatabaseService>().getFriendsLeaderboard(
                    friendsList,
                  ),
                  builder: (context, snapshot) {
                    if (!snapshot.hasData) {
...
class _HistoryScreenState extends State<HistoryScreen> {
  ...
  void initState() {
    super.initState();
    _historyFuture = locator<DatabaseService>().getHistoryLogs(
      widget.filterType,
    );
  }
      ...
      body: FutureBuilder<List<HistoryLog>>(
        ...
        builder: (context, snapshot) {
          ...
          if (historyItems.isEmpty) {
            return Center(
              child: Text(
                "You haven't watched any ${widget.filterType.toLowerCase()}s yet.",
...
class WatchLaterScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final watchLaterItems = context
        .watch<JournalProvider>()
        .activeItems
        .where((item) => item.isWatchLater && item.status != 'Completed')
        .toList();
    return Scaffold(
      appBar: AppBar(
        leading: IconButton(
...
class _CompletedMediaScreenState extends State<CompletedMediaScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_ratingFilter == 'Disliked') {
      completedItems = completedItems.where((i) => i.isLiked == false).toList();
    }
