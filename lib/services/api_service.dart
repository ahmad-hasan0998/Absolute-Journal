...
class ApiService {
  ...
  Future<List<Map<String, dynamic>>> _searchTMDB(
    String query,
    String requestedType,
  ) async {
    final url = Uri.parse(
      'https://api.themoviedb.org/3/search/multi?api_key=$_tmdbKey&query=$query',
    );
  ...
  Future<int> getTMDBTotalEpisodes(int showId) async {
    final url = Uri.parse(
      'https://api.themoviedb.org/3/tv/$showId?api_key=$_tmdbKey',
    );
    try {
      final response = await http.get(url);
      if (response.statusCode == 200) {
  ...
  Future<Map<int, List<Map<String, dynamic>>>> getPersonalizedSchedule(
    int year,
    int month,
