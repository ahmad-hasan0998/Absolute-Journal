...
class DatabaseService {
  ...
  Future<void> debugSetLastActivity(int daysAgo) async {
    ...
    await _db.collection('users').doc(currentUserId).update({'last_activity_date': Timestamp.fromDate(past)});
  }
  Future<void> debugSetLastReset(int daysAgo) async {
    if (currentUserId == null) return;
    DateTime past = DateTime.now().subtract(Duration(days: daysAgo));
    await _db.collection('users').doc(currentUserId).update({'last_reset_date': Timestamp.fromDate(past)});
  }
  ...
  Future<String?> addMediaToJournal(MediaItem item) async {
    ...
    var docRef = await _db.collection('users').doc(currentUserId).collection('my_journal').add({
      'title': item.title, 'type': item.type, 'status': item.status, 'progress': item.progress, 'total': item.total, 'posterUrl': item.posterUrl, 'isWatchLater': item.isWatchLater, 'isTop5': item.isTop5, 'completedCount': item.completedCount, 'isLiked': item.isLiked, 'timestamp': FieldValue.serverTimestamp(),
    });
    return docRef.id;
  }
  Future<List<MediaItem>> getUserJournal() async {
    ...
    return snapshot.docs.map((doc) {
      ...
      return MediaItem(id: doc.id, title: data['title'] ?? 'Unknown', type: data['type'] ?? 'Movie', status: data['status'] ?? 'Active', progress: data['progress'] ?? 0, total: data['total'] ?? 1, lastUpdated: 'Just now', timestamp: ts?.toDate() ?? DateTime.now(), posterUrl: data['posterUrl'], isWatchLater: data['isWatchLater'] == true, isTop5: data['isTop5'] == true, completedCount: data['completedCount'] ?? 0, isLiked: data['isLiked']);
    }).toList();
  }
  Future<void> updateMedia(MediaItem item) async {
    ...
    await _db.collection('users').doc(currentUserId).collection('my_journal').doc(item.id).update({
      'progress': item.progress, 'status': item.status, 'completedCount': item.completedCount, 'isWatchLater': item.isWatchLater, 'isTop5': item.isTop5, 'isLiked': item.isLiked, 'timestamp': FieldValue.serverTimestamp(),
    });
  }
  ...
  Future<void> addHistoryLog(HistoryLog log) async {
    if (currentUserId == null) return;
    await _db.collection('users').doc(currentUserId).collection('history_logs').add({
      'mediaId': log.mediaId, 'title': log.title, 'type': log.type, 'action': log.action, 'timestamp': Timestamp.fromDate(log.timestamp), 'posterUrl': log.posterUrl, 'isLiked': log.isLiked,
    });
  }
  Future<List<HistoryLog>> getHistoryLogs(String type) async {
    if (currentUserId == null) return [];
    var snapshot = await _db.collection('users').doc(currentUserId).collection('history_logs').orderBy('timestamp', descending: true).get();
    return snapshot.docs.map((doc) {
      var data = doc.data();
      return HistoryLog(id: doc.id, mediaId: data['mediaId'] ?? '', title: data['title'] ?? 'Unknown', type: data['type'] ?? 'Movie', action: data['action'] ?? '', timestamp: (data['timestamp'] as Timestamp).toDate(), posterUrl: data['posterUrl'], isLiked: data['isLiked']);
  ...
  Future<void> toggleFriend(String friendUid, bool isAdding) async {
    if (currentUserId == null) return;
    if (isAdding) { await _db.collection('users').doc(currentUserId).update({'friends': FieldValue.arrayUnion([friendUid])}); }
    else { await _db.collection('users').doc(currentUserId).update({'friends': FieldValue.arrayRemove([friendUid])}); }
  }
  ...
  Future<int> getUserPrevRank(String targetUid) async {
    if (currentUserId == null) return 999;
    var myDoc = await _db.collection('users').doc(currentUserId).get();
    List<dynamic> myFriends = myDoc.data()?['friends'] ?? [];
    List<String> idsToFetch = [currentUserId!, ...myFriends.map((e) => e.toString())];
  ...
  Future<void> addXP(int amount) async {
    ...
    await _db.runTransaction((transaction) async {
      ...
      transaction.update(userRef, {'xp_score': newXp, 'streak': streak, 'rank': _calculateRank(newXp), 'last_activity_date': FieldValue.serverTimestamp()});
    });
  }
